...
int main(){
	...
	while (nomer != exixt){
		if (nomer == exixt + 1){
			...
			sorting(arr, sz);
			view_arr(arr, sz);
			cout << endl << "Введите число, которое нужно найти: ";
			cin >> k;
			int i = iterat(arr, sz, k);
			if (i == -1){
				cout << "Искомого числа в массиве нет" << endl;
			}else{
				cout << "Нужный вым элемент находится на " << i + 1 << " позиции." << endl;
			}
			nomer = exixt + 1;
		}
		else if(nomer == 2){
			cout << "Введите строку, в которой нужно осуществить поиск: " << endl;
			char str[100];
			cin >> str;
			...
			if (alt >= 0){
				cout << "Количество знаков, на которое должна быть смещена строка: " << alt << endl;
			}
			else{
				cout << "Строки для поиска в изначальной строке нет" << endl;
			}
			nomer = exixt + 1;
		}
		else{
